...
class SigesNewSupabaseUser extends BaseAuthUser {
  SigesNewSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> sigesNewSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SigesNewSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
